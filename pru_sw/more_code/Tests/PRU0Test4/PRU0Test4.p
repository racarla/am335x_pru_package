...
#define ValueMask r25
#define SHARED_MID_RAM_SIZE r26
...
TOP:
  ...
  MOV SHARED_MID_RAM_SIZE, SHARED_RAM_ADDRESS + SHD_RAM_ADR_BUFF_MID
  MOV SHARED_RAM_SIZE, SHARED_RAM_ADDRESS + SHD_RAM_ADR_BUFF_FI

  // Numero de ciclos por 8 muestras
  MOV Time3, 2000 // 5 ns * 2000 = 10 uS
...
CAPTURE:
  // Load time value
  LBBO Time1, PRU_POINTER, PRU_CYCLES_COUNTER, 4
  // Save time cycles in shared_ram
  SBBO Time1, SHARED_RAM, 0, 4
  ADD SHARED_RAM, SHARED_RAM, 4

  // write STEPCONFIG register (this triggers capture)
  MOV tmp0, 0x1fe
  SBBO tmp0, adc_, STEPCONFIG, 4
...
READ_ALL_FIFO0:  // lets read all fifo content and dispatch depending on pin type
  ...
  AND value, value, ValueMask

  SBBO value, shared_ram_pointer, 0, 2
  ADD shared_ram_pointer, shared_ram_pointer, 2
  ADD tmp0, tmp0, 1
  QBGT READ_ALL_FIFO0, tmp0, 8

WAIT:
  LBBO Time2, PRU_POINTER, PRU_CYCLES_COUNTER, 4     // Load "before" cycle count into R1
  SUB tmp0, Time2, Time1
  QBGT WAIT, tmp0, Time3

  QBLE FIN, shared_ram_pointer, SHARED_MID_RAM_SIZE
